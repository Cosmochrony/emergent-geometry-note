...
takes the metric established here as its starting point.
  ...
  \section{Constituent Papers}
    ...
    \subsection{Continuum limit and one-dimensional shadow: Q5a and Q5a-O2}
      ...
      BFS fingerprints and selected Gram--Schmidt vectors are pure Fourier modes, with realised
      frequencies $o(q)$; the three-coordinate reduction is a truncation fixed in advance
      ($\mathrm{HEFF\_DIM}=3$), from which no admissible rank can be measured; and the
      admissibility form is $O(q^{-2})$ on normalised modes, which rules out the $q^{-1}$
      coercivity scale.
